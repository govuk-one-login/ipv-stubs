...
import { TestUser, DocumentType, EvidenceType } from "./managementEnqueueVcRequest";

export function buildMockVc(userId: string, testUser: TestUser, documentType: DocumentType, evidenceType: EvidenceType, ci: string[] = []) {
  const timestamp =  Math.round(new Date().getTime() / 1000);
  return {
    iss: "", // TODO get from config
    aud: "", // TODO get from config
    sub: userId,
    iat: timestamp,
    nbf: timestamp,
    vc: {
