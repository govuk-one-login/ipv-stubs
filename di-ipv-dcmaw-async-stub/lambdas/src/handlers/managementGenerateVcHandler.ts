...
import { isManagementEnqueueVcRequestEvidenceAndSubject } from "../domain/managementEnqueueRequest";
  ...
  try {
    ...
    if (!isManagementEnqueueVcRequestEvidenceAndSubject(requestBody)) {
      return buildApiResponse(
        { errorMessage: "Unrecognised request body" },
        400,
      );
    }
    ...
    return buildApiResponseFromString(signedJwt, 200);
  } catch (error) {
    return buildApiResponse(
