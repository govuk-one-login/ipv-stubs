...
public class ApiGatewayProxyEventHelper {
    ...
    public static APIGatewayProxyResponseEvent generateAPIGatewayProxyResponseEvent(
    ...
    private static String getHeaderByKey(
            String key, APIGatewayProxyRequestEvent input, boolean isRequired)
            throws FailedToParseRequestException {
        ...
        if (isRequired && StringUtils.isBlank(headerValue)) {
            throw new FailedToParseRequestException(
                    String.format("%s in request headers is empty", key));
        }
    ...
    public static String getRequiredQueryParamByKey(String key, APIGatewayProxyRequestEvent input)
        ...
        if (StringUtils.isBlank(queryParamValue)) {
            throw new FailedToParseRequestException(
                    String.format("%s in request query params is empty", key));
        }
