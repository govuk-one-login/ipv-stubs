...
export const handler: Handler = async (
    ...
    if (ticfRequest == null) return apiResponses._400({errorMessage: "Pls. pass proper request."});

    // preparing response
    let ticfSigningKey : string | null | undefined = await getSsmParameter(process.env.TICF_PARAM_BASE_PATH + 'signingKey');
    if (!ticfSigningKey) {
      return apiResponses._500({errorMessage: "Error while retrieving TicF CRI VC signing key."});
    }
    let timeoutVC : string | null | undefined = await getSsmParameter(process.env.TICF_PARAM_BASE_PATH + 'timeoutVC');
    timeoutVC ??= "false";
    let includeCIToVC : string | null | undefined = await getSsmParameter(process.env.TICF_PARAM_BASE_PATH + 'includeCIToVC');
    includeCIToVC ??= "false";

    let buildJwtParams: SignedJwtParams;
    buildJwtParams = {
        issuer: process.env.ISSUER,
        customClaims: getCustomClaims(JSON.parse(timeoutVC.toLowerCase()), JSON.parse(includeCIToVC.toLowerCase()), ticfRequest.sub),
...
function getCustomClaims(timeOutVC: boolean, includeCIToVC: boolean, userId: string): JWTPayload {
    if (timeOutVC) {
      if (includeCIToVC) {
        return {
          sub: userId,
